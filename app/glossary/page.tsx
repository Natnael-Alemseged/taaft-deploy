...
// import { Footer } from "react-day-picker" // Correct this import if Footer is not from react-day-picker
import Footer from "@/components/ui/footer"; // Assuming Footer component path from your project

import { useGlossaryGrouped } from "@/hooks/use-glossary" // Import the new hook
import type { GlossaryTerm, GroupedGlossaryResponse } from "@/services/glossary-service"; // Import types
...
export default function Glossary() {
  ...
  const renderTermCard = (term: GlossaryTerm) => (
      <div key={term.id} className="border border-[#e5e7eb] rounded-lg p-4 md:p-6 hover:shadow-sm transition-shadow">
        ...
        <Link href={`/terms/${slugify(term.name)}`} className="block">
          ...
          {/* Optional: Add 'Read more' link if /terms/[slug] page exists */}
  ...
  if (isError) {
    return (
        <div className="min-h-screen bg-white flex flex-col items-center justify-center text-red-600">
          ...
          <Button onClick={() => window.location.reload()} className="bg-red-500 hover:bg-red-600 text-white">Retry Loading</Button>
        </div>
    );
  }

  // Render loading state
  if (isLoading || !groupedGlossaryData) { // Also show loading if data is null initially
    return (
  ...
  return (
      <>
        ...
        <div className="min-h-screen bg-white dark:bg-gray-950"> {/* Adjusted background */}
          <main className="max-w-6xl mx-auto px-4 py-8">
            <div className="text-center mb-8">
              <h1 className="text-2xl md:text-3xl font-bold text-[#111827] dark:text-white mb-3">AI Tools Glossary</h1> {/* Adjusted text color */}
              <p className="text-sm md:text-base text-[#6b7280] dark:text-gray-400 max-w-2xl mx-auto"> {/* Adjusted text color */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="hidden md:block md:col-span-1">
                <div className="sticky top-8"> {/* Adjusted top value if needed */}
                  <h2 className="font-semibold text-[#111827] dark:text-white mb-4">Contents</h2> {/* Adjusted text color */}
...
function AlphabeticalNavigationMobile({
  ...
  return (
      <div className="border border-[#e5e7eb] dark:border-gray-700 rounded-lg overflow-hidden"> {/* Adjusted border */}
        <button
        ...
        {isOpen && (
            <div className="p-3">
              {alphabetRows.map((row, rowIndex) => (
                  <div key={rowIndex} className="flex mb-2 last:mb-0 gap-1"> {/* Added gap */}
                    {row.map((letter) => {
                      const hasContent = lettersWithContent.includes(letter);
                      const isSelected = selectedLetter === letter;
                      ...
                      return (
                          <button
                              key={letter}
                              onClick={() => { onLetterSelect(letter); setIsOpen(false); /* Close on select */ }}
                              disabled={!hasContent} // Disable button if no content for this letter
                    ...
                    {row.length < 8 &&
                        Array(8 - row.length).fill(0).map((_, i) => <div key={i} className="flex-1 h-8"></div>)} {/* Added height */}
                  </div>
              ))}
            </div>
        )}
