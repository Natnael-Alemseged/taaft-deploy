...
export default function ChatInterface({ isOpen, onClose, inputRef, isRelativeToParent = false }: ChatInterfaceProps) {
        ...
        const sendPendingMessage = async () => {
          if (!activeChatId) {
            try {
              ...
              // Send the pending message with isDirectSearch: false (default)
  ...
  // Added isDirectSearch parameter with a default value of false
  async function handleSendMessage(content?: string, isDirectSearch: boolean = false) {
    ...
    try {
      ...
      console.log(`Sending message to chat ID: ${chatId}, isDirectSearch: ${isDirectSearch}`)

      // Pass the isDirectSearch flag to the mutation
      const response = await chatCompletion.mutateAsync({
        sessionId: chatId,
        message: messageContent,
        model: "gpt4",
        systemPrompt: "You are a helpful assistant.",
        metadata: {  }, // Include isDirectSearch in metadata -- This was the previous incorrect place
        isDirectSearch: isDirectSearch // Pass isDirectSearch directly to the hook's mutate function
      })
  ...
  const handleToolRecommendationClick = async (tool: string) => {
    // Send the recommendation text as a message, not as a direct search
    handleSendMessage(tool, false) // Explicitly set isDirectSearch to false
  }
          ...
          <div className="flex h-[500px] flex-col rounded-xl border border-gray-200">
            ...
            <div className="border-t border-gray-100 p-4">
              <div className="flex items-center space-x-2">
                <div className="relative flex-1">
                  ...
                  <Input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault()
                          // Call handleSendMessage with isDirectSearch: false (default)
                          handleSendMessage();
                        }
                      }}
                  />
                ...
                <Button
                    onClick={() => {
                      // Call handleSendMessage with isDirectSearch: true
                      handleSendMessage(undefined, true); // Pass undefined for content to use current input state
                    }}
