...
const getBadgeClass = (label: string) => {
    switch (label?.toLowerCase()) { // Use toLowerCase for consistent matching
        case "premium":
        case "subscription":
        case "one-time":
        case "usage-based":
...
export default function ToolCard({ tool }: ToolCardProps) {
    ...
    const pricingText = tool.pricing?.charAt(0).toUpperCase() + tool.pricing?.slice(1) || "Unknown";


    return (
        // Card component acts as the container
        <Card className="border border-[#e5e7eb] dark:border-gray-800 rounded-lg overflow-hidden bg-white dark:bg-gray-900 shadow-lg w-full h-full flex flex-col"> {/* Added dark mode, flex-col */}
            <CardContent className="p-4 flex flex-col flex-grow"> {/* Added flex-col flex-grow */}
                <div className="mb-2 flex items-center justify-between">
                    <div className="flex gap-2">
                        {/* Category Badge */}
                        {/* You might want a specific color logic for categories */}
                        <span className="text-xs px-3 py-1 bg-[#f5f0ff] text-[#a855f7] dark:bg-purple-900/20 dark:text-purple-400 rounded-full"> {/* Added dark mode */}
                            {tool.category}
            </span>
                        {/* Pricing Badge */}
                        {tool.pricing && (
                    ...
                    {/* Optional: Add a checkmark or dollar sign icon based on pricing */}
                                {tool.pricing === 'free' ? <span className="mr-0.5">✓</span> : tool.pricing !== 'unknown' && <span className="mr-0.5">$</span>}
                                {pricingText}
                </span>
                        )}
                    </div>
                    ...
                    {/* {tool.isFeatured && (
               <span className={`text-xs px-2 py-0.5 rounded-full ${getBadgeClass("featured")}`}>
                   Featured
               </span>
          )} */}
                </div>
                ...
                {/* Tool Name */}
                <h3 className="text-lg font-semibold text-[#111827] dark:text-white mb-2">{tool.name}</h3> {/* Added dark mode */}

                {/* Tool Description */}
                <p className="text-sm text-[#4b5563] dark:text-gray-400 mb-4 flex-grow"> {/* Added dark mode, flex-grow */}
                    {tool.description}
                </p>
                ...
                {tool.tags && Array.isArray(tool.tags) && tool.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                        {tool.tags.map((tag, index) => (
                            <span key={index} className="text-xs px-2 py-0.5 bg-[#f3f4f6] dark:bg-gray-800 text-[#6b7280] dark:text-gray-400 rounded"> {/* Added dark mode */}
                                {tag}
                    </span>
                        ))}
                    </div>
                ...
                <div className="flex justify-between items-center mt-auto"> {/* Use mt-auto to push to bottom */}
                    <div className="flex space-x-2">
                        ...
                        <button className="p-1.5 border border-[#e5e7eb] dark:border-gray-700 rounded text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800"> {/* Added dark mode */}
